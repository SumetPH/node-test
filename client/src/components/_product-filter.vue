  ...
  <div>
      ...
      <div class="col s12">
        <ul class="collection" style="border:0;">
          <form @change.prevent="categoryFilter">
            <li class="collection-item" v-for="(item, index) in categoryList" :key="index">
              <label>
                <input class="filled-in" type="checkbox" v-model="category" :value="item" />
                <span>
                  {{ item }}
                </span>
              </label>
    ...
    <div class="row">
      ...
      <div class="col s12 m6">
        <input class="form-control" type="number" @change="minMaxPrice" v-model="min" placeholder="ต่ำสุด" min="0" />
      </div>
      <div class="col s12 m6 p-1">
        <input type="number" @change="minMaxPrice" v-model="max" placeholder="สูงสุด" min="0" />
      </div>
    </div>
